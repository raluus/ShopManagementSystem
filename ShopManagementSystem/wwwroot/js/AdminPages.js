...
fetch('/Json/additionalAttributesData.json')
    .then(response => response.json())
    .then(data => {
        // Extract the data from the JSON object
        const attributes = data.attributes;
        const nutri = data.nutritionalInformation;
        var maxAllowedInputs = attributes.length; 
        ...
        document.getElementById('addAttributeButton').addEventListener('click', () => {
            if (currentGeneratedInputs < maxAllowedInputs) {
                ...
                populateOptions(select, attributes);
                //select.setAttribute("asp-for", "ProductAttributes.AttributeKey");

                var input = document.createElement("input");
                //input.setAttribute("asp-for", "ProductAttributes.AttributeValue");
                input.classList.add("form-control");
                input.setAttribute("type", "text");

                var validationSpan = document.createElement("span");
                validationSpan.setAttribute("asp-validation-for", "ProductAttributes.AttributeValue");
